...
public class SenkuTable {
    ...
    public int finishGame() {
        if (active == 1 && !existMove()) {
            //si solo queda una ficha y no hay movimientos posibles, se ha ganado
            return 1;
        }
        if (active > 1 && !existMove()) {
            //si quedan mas de una ficha y no hay movimientos posibles, se ha perdido
            return 0;
        }
        //si no se ha ganado ni perdido, se devuelve -1
        return -1;
    }
